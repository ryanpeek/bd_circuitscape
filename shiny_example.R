...
shapefile <- sf::st_read("~/Downloads/ca_ag_regions.shp")

shapefile <- sf::st_as_sf(shapefile)  %>%
  mutate(fill_color = if_else(!is.na(region), "#1C00ff00", "#A9A9A9"))

# return data if point intersects shapefile
region_data <- function(shapefile, markers) {
  ...
  # intersect data
  datout <- as.data.frame(shapefile)[which(sapply(sf::st_intersects(shapefile,dat), 
                                                  function(z) 
                                                    if (length(z)==0) NA_integer_ else z[1]) == 1), ]
  return(datout)
}
